...
export default function SorteioPublico() {
  ...
  async function runAnimation(images: string[], winnerUrl: string) {
    ...
    // dá “voltas” e desacelera (UX: sensação de roleta real)
    const totalSteps = n * 4 + Math.floor(Math.random() * n);
    ...
    for (let step = 0; step < totalSteps; step++) {
      ...
      const delay = 35 + Math.floor(220 * t * t);
      await sleep(delay);
    }

    // garante parar no vencedor
    while (cur !== winnerIndex) {
      cur = (cur + 1) % n;
      setActiveIndex(cur);
      await sleep(240);
    }
    ...
    (async () => {
      ...
      while (alive) {
        ...
        if (
          ...
          // roda animação com o grid completo (antes de renderizar só vencedor)
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-4 text-2xl font-extrabold tracking-tight md:text-3xl">
              Sorteio ao vivo
              <span className="ml-2 text-[#ff5c0c]">•</span>
            </h1>
          ...
          <motion.div
            ...
            className="w-full rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur md:w-[360px]"
          >
            {!data ? (
